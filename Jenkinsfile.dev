node {
    ...
    properties([
        parameters([
            ...
            ,
            booleanParam(name: 'BUILD_DOCKER', defaultValue: true, description: 'Construir y desplegar imágenes Docker (activado por defecto para DEV)')
        ])
    ])
    ...
    try {
        stage('Checkout') {
            ...
            try {
                ...
                // Verificación adicional: si estamos en la rama dev, forzar el nombre
        ...
        stage('Build Backend') {
            echo "🔨 Construyendo Backend para DEV..."
            dir('backend') {
                sh '''
        ...
        stage('Build Frontend') {
            echo "🎨 Construyendo Frontend para DEV..."
            sh '''
        ...
        stage('SonarQube Analysis') {
            echo "📊 Ejecutando análisis de SonarQube para DEV..."
            // IMPORTANTE: El nombre debe coincidir con el configurado en "Manage Jenkins > System > SonarQube servers"
            withSonarQubeEnv('SonarQube') {
                withCredentials([string(credentialsId: 'sonarqube-token', variable: 'SONAR_TOKEN')]) {
                    // ANÁLISIS DEL BACKEND (con cobertura de tests y configuración específica para dev)
                    ...
                    sh '''
                        ...
                        echo "=== Análisis de SonarQube para BACKEND DEV completado ==="
                    '''
                    
                    // ANÁLISIS DEL FRONTEND (con configuración específica para dev)
                    ...
                    sh '''
                        ...
                        echo "=== Análisis de SonarQube para FRONTEND DEV completado ==="
                    '''
                }
            }
        }
        ...
        stage('Deploy DEV') {
            if (params.BUILD_DOCKER) {
                ...
                echo "   🧹 Limpiando contenedores de DEV existentes..."
                sh '''
                  ...
                  echo "🛑 Deteniendo contenedores de DEV..."
                ...
                echo "   🐳 Construyendo y desplegando contenedores de DEV..."
                sh '''
        ...
        stage('Send Technical Debt Report') {
            ...
            try {
                sh '''
                    echo "=== Enviando Reporte de Deuda Técnica DEV ==="
...
📈 PROYECTOS SONARQUBE DEV:
- Backend: hospital-backend-dev
- Frontend: hospital-frontend-dev

📧 REPORTE DE DEUDA TÉCNICA:
Se ha enviado automáticamente el reporte de deuda técnica con indicador [RAMA DEV].
...
"""
            ...
            echo "✅ Notificación de éxito DEV enviada a: ${recipients}"
        } catch (err) {
        ...
        // Error handling
        if (env.CHANGE_ID) {
            echo "❌ Pull Request #${env.CHANGE_ID} DEV falló: ${e.getMessage()}"
        } else {
            echo "❌ Pipeline DEV falló en rama ${env.BRANCH_NAME}: ${e.getMessage()}"
        }
        // Notificación por correo a Lead Developer y Product Owner
        try {
            def recipients = 'jflores@unis.edu.gt, jnajar@unis.edu.gt'
            def subject = (env.CHANGE_ID ? "PR #${env.CHANGE_ID} DEV falló: ${env.JOB_NAME} #${env.BUILD_NUMBER}" : "Pipeline DEV falló: ${env.JOB_NAME} #${env.BUILD_NUMBER} (Rama: ${env.BRANCH_NAME})")
...
3. Revisar métricas de SonarQube DEV
...
"""
            ...
            echo "📧 Notificación de fallo DEV enviada (emailext) a: ${recipients}"
        } catch (err) {
            echo "⚠️ No se pudo enviar la notificación por correo: ${err}"
        }
        throw e
